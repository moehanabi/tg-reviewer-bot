...
async def remove_decision(submission_meta, reviewer) -> tuple[dict, bool]:
    if reviewer in submission_meta["reviewer"]:
        ...
        if submission_meta["reviewer"][reviewer][2] in [
            ...
            await Reviewer.count_modify(reviewer, "approve_count", -1)
        else:
            await Reviewer.count_modify(reviewer, "reject_count", -1)
...
async def retract_approved_submission(
    ...
    try:
        ...
        # modify stats data
        await Submitter.count_modify(submission_meta["submitter"][0], "approved_count", -1)
