    ...
    inline_keyboard = InlineKeyboardMarkup(
        [
            ...
            [
                InlineKeyboardButton(
                    "📝 添加备注",
                    switch_inline_query_current_chat="/append 请回复原消息并修改此处文字",
                ),
                InlineKeyboardButton(
                    "⬅️ 删除备注",
                    switch_inline_query_current_chat="/remove_append 请回复原消息并修改此处文字为待删除备注的编号",
                ),
            ],
            [
                InlineKeyboardButton(
...
def generate_submission_meta_string(submission_meta):
    ...
    status_title = (
        "❔ 待审稿件"
        if status == SubmissionStatus.PENDING
        else ("✅ 已通过稿件" if status == SubmissionStatus.APPROVED else "❌ 已拒绝稿件")
    )
    # tags
